...
def Carregamento():
  try:
    ...
    return main()
def confirmador(N,nome):  #O confirmador é a função mais essêncial, ela confirma se a resposta do usuários é válida
  confirmado=False
  ...
  def habilidade(self):
    if self.nome=="Lander":
      ...
      for i in range(0,len(time_inimigo)):
        time_inimigo[i].vida=time_inimigo[i].vida+20
      print("Lander cura 20 de vida de todos os aliados")
    if self.nome=="Draahl":
...
class personagem(abc.ABC): #Aqui é a base para os métodos 
  @abc.abstractmethod
  def descansar(self):
    pass
  @abc.abstractmethod
...
def habilidade1(pessoa):
  ...
  if pessoa.Habilidade1=="Prece de cura" and pessoa.mana>=5:
    for i in range (0,len(time)):
      if time[i].vigor>0:
        time[i].vigor=time[i].vigor+5    
    print(pessoa.nome+" curou o grupo em 5 pontos de vida")
    pessoa.mana=pessoa.mana-5  
  else:
    if pessoa.Habilidade1=="Prece de cura" and pessoa.mana<5:
...
def combate3():
  ...
  time_inimigo=[]
  GregMorthos=Knight_of_the_round("Greg Morthos",75,30,0)
  time_inimigo.append(GregMorthos)
  Lander=Knight_of_the_round("Lander",90,20,0)
  time_inimigo.append(Lander)
  Kael=Knight_of_the_round("Kael",60,15,0)
  time_inimigo.append(Kael)
  Draahl=Knight_of_the_round("Draahl",80,25,0)
...
def seletor_de_equipe():   #Aqui o usuário escolhe sua equipe
  ...
  dentro_do_time5=0
  Strangelove=mago("Strangelove","Mago Vermelho",11,11,9,7,"Prece de cura","Disparo de fogo",7,1,0)
...
def criador_de_personagem():  #Aqui o personagem do jogador é criado
  ...
  while Vigor_do_player+Mana_do_player+Energia_do_player+Agilidade_do_player>40 or Vigor_do_player+Mana_do_player+Energia_do_player+Agilidade_do_player<40:
    confirmado=True
  ...
  while Contador_Habilidade1_do_player==0:
    ...
    print("Um feitiço de cura rápida, que restaura 5 pontos de vida de todos os aliados, ao custo de 5 de mana")
